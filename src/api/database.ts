...
import { ChatsPostgresClient } from '@/storage/chats_postgres_client';
import { DatabaseConfiguration } from '@/models/configuration';
import { PoolClient } from 'pg';

let databaseClient: ChatsDatabaseClient | null = null;

const initializeDatabase = async (
    config: DatabaseConfiguration,
    registerMigrations?: Map<number, (client: PoolClient) => Promise<void>> 
): Promise<void> => {
    databaseClient = new ChatsPostgresClient(config);
...
const getDatabaseClient = (): ChatsDatabaseClient | null => {
