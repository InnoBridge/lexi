...
const GET_SCHEMA_VERSION_QUERY = 
    `SELECT COALESCE(MAX(version), 0) as version FROM messenger_schema_versions`;
...
const INDEX_CHATS_QUERY = 
    `CREATE INDEX IF NOT EXISTS idx_messenger_chats_user_id1 ON messenger_chats(user_id1);
     CREATE INDEX IF NOT EXISTS idx_messenger_chats_user_id2 ON messenger_chats(user_id2);
     CREATE INDEX IF NOT EXISTS idx_messenger_chats_updated_at ON messenger_chats(updated_at);
     CREATE INDEX IF NOT EXISTS idx_messenger_chats_created_at ON messenger_chats(created_at);`;
...
const CREATE_MESSAGES_TABLE_QUERY = 
    `CREATE TABLE IF NOT EXISTS messenger_messages (
        ...
        created_at     TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        CONSTRAINT fk_messenger_messages_chat 
            FOREIGN KEY (chat_id) REFERENCES messenger_chats(chat_id) ON DELETE CASCADE
    )`;

const INDEX_MESSAGES_QUERY =
    `CREATE INDEX IF NOT EXISTS idx_messenger_messages_chat_id ON messenger_messages(chat_id);
     CREATE INDEX IF NOT EXISTS idx_messenger_messages_created_at ON messenger_messages(created_at);
     CREATE INDEX IF NOT EXISTS idx_messenger_messages_chat_created ON messenger_messages(chat_id, created_at DESC);
...
const ADD_CHAT_QUERY = 
    `INSERT INTO messenger_chats (chat_id, connection_id, user_id1, user_id2)
     VALUES ($1, $2, $3, $4)
...
const DELETE_CHAT_QUERY = 
    `DELETE FROM messenger_chats WHERE chat_id = $1`;
...
const GET_MESSAGES_BY_USER_ID_QUERY = 
    `SELECT m.message_id, m.chat_id, m.sender_id, m.content, m.created_at
     ...
     JOIN messenger_chats c ON m.chat_id = c.chat_id
...
const ADD_MESSAGE_QUERY = 
    `INSERT INTO messenger_messages (message_id, chat_id, sender_id, content)
     VALUES ($1, $2, $3, $4)
